...
class FifthStepBlock extends React.PureComponent<PropsType> {
  showModal = () => {
  ...
  render (): React.Element<'div'> {
    ...
    if (step.make_payment) {
      ...
      hint = (
        <p className='m-t-2 c-maj-step-block__step__hint'>
    ...
    return (
      <div className={`c-maj-step-block__container  ${active === 1 ? 'is-active' : ''}`}>
        <article className={`c-maj-step-block__step ${done ? 'is-done' : ''} ${active === 1 ? 'is-active' : ''}`}>
          <span className={`c-maj-step-block__order ${active === 1 ? 'is-active' : ''}`}>
