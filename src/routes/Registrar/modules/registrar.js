...
export const GET_CONFORMITY_DAYS_REQUEST = 'GET_CONFORMITY_DAYS_REQUEST'
export const GET_CONFORMITY_DAYS_SUCCESS = 'GET_CONFORMITY_DAYS_SUCCESS'

export const STORE_CONFORMITY_DAY_REQUEST = 'STORE_CONFORMITY_DAY_REQUEST'
export const STORE_CONFORMITY_DAY_SUCCESS = 'STORE_CONFORMITY_DAY_SUCCESS'

export const GET_EXAM_RULES_REQUEST = 'GET_EXAM_RULES_REQUEST'
export const GET_EXAM_RULES_SUCCESS = 'GET_EXAM_RULES_SUCCESS'
...
export const GET_REGISTRAR_EXAMS_REQUEST = 'GET_REGISTRATION_EXAMS_REQUEST'
export const GET_REGISTRAR_EXAMS_SUCCESS = 'GET_REGISTRATION_EXAMS_SUCCESS'
...
// store conformity days
export function storeConformityDayRequest () {
  return {
    type: STORE_CONFORMITY_DAY_REQUEST
  }
}

export function storeConformityDaySuccess (days) {
  return {
    type: STORE_CONFORMITY_DAY_SUCCESS,
    payload: days
  }
}

export function storeConformityDay ({ dayId }) {
  return function (dispatch, getState) {
    dispatch(storeConformityDayRequest())
    const state = getState()
    const token = state.auth.registrartoken
    return request.post(`${APIBASE}/api/registrar/conformity_choose`)
      ...
      .end((err, res) => {
        if (!err && res.ok) {
          dispatch(storeConformityDaySuccess(res.body))
        }
      })
  }
}
// get conformity days
export function getConformityDaysRequest () {
  return {
    type: GET_CONFORMITY_DAYS_REQUEST
  }
}

export function getConformityDaysSuccess (days) {
  return {
    type: GET_CONFORMITY_DAYS_SUCCESS,
    payload: days
  }
}

export function getConformityDays () {
  return function (dispatch, getState) {
    dispatch(getConformityDaysRequest())
    const state = getState()
    const token = state.auth.registrartoken
    return request.get(`${APIBASE}/api/registrar/conformity_days`)
      .set('x-access-token', token)
      .set('Accept', 'application/json')
      .set('Content-Type', 'application/json')
      .end((err, res) => {
        if (!err && res.ok) {
          dispatch(getConformityDaysSuccess(res.body))
        }
      })
  }
}
...
export function getExams () {
  return function (dispatch, getState) {
    ...
    const state = getState()
    const token = state.auth.registrartoken
    return request.get(`${APIBASE}/api/registrar/exams`)
      .set('Authorization', 'Bearer ' + token)
      .set('Accept', 'application/json')
      .set('Content-Type', 'application/json')
      .end((err, res) => {
        if (!err && res.ok) {
          dispatch(getExamsSuccess(res.body))
        }
      })
  }
}
...
export function getExamsRequest () {
  return {
    type: GET_REGISTRAR_EXAMS_REQUEST
  }
}

export function getExamsSuccess (exams) {
  return {
    type: GET_REGISTRAR_EXAMS_SUCCESS,
    payload: exams
  }
}
...
export function getExamRules () {
  return function (dispatch, getState) {
    dispatch(getExamRulesRequest())
    const state = getState()
    const token = state.auth.registrartoken
    return request.get(`${APIBASE}/api/registrar/exam_rules`)
      .set('Authorization', 'Bearer ' + token)
      .set('Accept', 'application/json')
      .set('Content-Type', 'application/json')
      .end((err, res) => {
        if (!err && res.ok) {
          dispatch(getExamRulesSuccess(res.body))
        }
      })
  }
}

export function getExamRulesRequest () {
  return {
    type: GET_EXAM_RULES_REQUEST
  }
}

export function getExamRulesSuccess (rules) {
  return {
    type: GET_EXAM_RULES_SUCCESS,
    payload: rules
  }
}
...
export function getProfileSuccess (profile) {
  ...
  return function (dispatch, getState) {
    ...
    const hideFees = profile.step ? profile.step.hide_fees : 0

    if (profile.fees_amount <= 0 && filesdone >= 0 && !feesExemption && notCanceled && !hideFees) {
...
const ACTION_HANDLERS = {
  ...
  [GET_CONFORMITY_DAYS_REQUEST]: (state: Object, action: Object): Object => ({
    ...state,
    loadingConformityDays: true
  }),
  [STORE_CONFORMITY_DAY_REQUEST]: (state: Object, action: Object): Object => ({
    ...state,
    loadingConformityDays: true
  }),
  [GET_CONFORMITY_DAYS_SUCCESS]: (state: Object, action: Object): Object => ({
    ...state,
    conformityDays: action.payload,
    loadingConformityDays: false
  }),
  [STORE_CONFORMITY_DAY_SUCCESS]: (state: Object, action: Object): Object => ({
    ...state,
    conformityDays: action.payload,
    loadingConformityDays: false
  }),
  ...
  [GET_REGISTRAR_EXAMS_REQUEST]: (state, action) => Object.assign({}, state, { examsLoading: true }),
  [GET_REGISTRAR_EXAMS_SUCCESS]: (state, action) => Object.assign({}, state,
    {
      exams: action.payload,
      examsLoading: false }
    ),
  [GET_EXAM_RULES_REQUEST]: (state, action) => Object.assign({}, state, { examRulesLoading: true }),
  [GET_EXAM_RULES_SUCCESS]: (state, action) => Object.assign({}, state,
    {
      examRules: action.payload,
      examRulesLoading: false }
    ),
