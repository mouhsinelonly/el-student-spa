  ...
  return <div className='text-xs-center MarketBookBlock shadow-1'>
    ...
    <div className='MarketBookBlock__details p-y-1 p-x-1'>
      ...
      <div className='MarketBookBlock__details-extra font-helvetica'>
        ...
        {!available ? <span>سوف يتم توفيره الفصل القادم</span> : null }
      </div>
    </div>
    <button disabled={!available} onClick={onClick} className='btn btn-success btn-block'>
      حجز
      {inCart ? <span className='MarketBookBlock__incart'>{inCart} <i className='material-icons'>shopping_basket</i></span> : null }
    </button>
  </div>
}
