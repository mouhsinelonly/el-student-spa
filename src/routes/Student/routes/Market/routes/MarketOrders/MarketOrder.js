...
import { deleteOrder, updateOrder } from 'routes/Student/modules/market'
...
const MarketOrder = ({ canCancel, id, status, code, createdAt, amount, cityName, stateName, ship, items }: PropertiesType) => {
  const [editShippingVisible, setEeditShippingVisiblity] = useState(false)
  const toggleShippingVisiblity = () => {
    setEeditShippingVisiblity(!editShippingVisible)
  }
  const handleSubmitShipping = (event: Object) => {
    setEeditShippingVisiblity(false)
    dispatch(updateOrder({ id: +event.target.dataset.id }))
  }
  ...
  return <div className='col-xs-12' key={id}>
    <div className='my-panel-white shadow-1 m-b-2 MarketOrders__item'>
      ...
      <footer>
        {(status === 'waiting' && canCancel)
          ? <button onClick={onDeleteClick} className='btn btn-danger-outline pull-xs-left m-a-2'>
            <i style={inlineStyle} className='material-icons'>delete</i> إلغاء الطلب
          </button> : null }
        {(['processing', 'waiting'].includes(status))
          ? <button onClick={toggleShippingVisiblity} className='btn btn-white-outline pull-xs-left m-a-2'>
            <i style={inlineStyle} className='material-icons'>{editShippingVisible ? 'keyboard_arrow_up' : 'keyboard_arrow_down'}</i> تعديل وسيلة الاستلام
          </button> : null }
      </footer>
      <div className='clearfix' />
      { editShippingVisible ? <ShippingInfo orderId={id} onSubmit={handleSubmitShipping} /> : null }
    </div>
  </div>
}
