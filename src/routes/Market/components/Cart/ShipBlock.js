...
import { getStates, setState, setMobile } from 'modules/market'
...
const ShipBlock = ({ onClick, active, amount }: PropertiesType): React.Element<'div'> => {
  const dispatch = useDispatch()
  const { cities, states, stateId, cityId, mobile } = useSelector((state: Object): Object => state.market)
  ...
  return <fieldset
    className={`MarketShipBlock is-shipping p-t-2 MarketCart__delivery ${active ? 'is-active' : null} m-t-1`}>
    <button data-id={item.id} onClick={onClick} type='button'
      className={`
              text-xs-right
